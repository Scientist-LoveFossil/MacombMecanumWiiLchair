...
AF_DCMotor motor(3);//, MOTOR12_2KHZ); // create motor #1
...
int chairDir = FORWARD; // 1 = FORWARD, 2 = BACKWARD
int throttle = 0;       // adjusted joystick reading
int speedLimit = 225;   // max motor set speed (up to 255)
...
void setup()
{
    ...
    if (debug){      // print intial readings
      Serial.print("\nWiiChuk Chair Controller Ver.2\n");
...
void loop()
{
  ...
  if (accx != 0 ){ // Chuk plugged in
    ...
    if (throttle < -deadBand && chairDir == FORWARD){ // slow down
      speedSet = 0;
    ...
    else if (throttle > deadBand && chairDir == BACKWARD){ // slow down
      speedSet = 0;
      cruise = 0;  // turn off cruise control
      drag = brakeDrag;
      if (curSpeed < deadBand){ // switch direction
